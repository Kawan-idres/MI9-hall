...
const PAYMENT_METHODS = [
  { id: 'fib',    label: 'FIB',        iconSrc: '/assets/FIB.png', desc: 'First Iraqi Bank',         color: '#00A651' },
  { id: 'super',  label: 'Super App',  iconSrc: '/assets/Super-Key.png', desc: 'Supercel / Key Card',      color: '#0066CC' },
  { id: 'master', label: 'Mastercard', iconSrc: '/assets/Master-Card.png', desc: 'Credit or debit card',     color: '#EB001B' },
]
...
export default function Payment({ reservation, onConfirm, onBack }) {
  const [step, setStep] = useState(2) // 2=details 3=payment 4=processing
  const [pm, setPm] = useState('fib')
  const [form, setForm] = useState({ name: '', phone: '', card: '', expiry: '', cvv: '', fib: '', super: '' })
  ...
  function validatePayment() {
    const e = {}
    if (pm === 'master' && !form.card) e.card = 'Required'
  ...
  function pay() {
    ...
    setStep(4)
    setTimeout(() => onConfirm({ table, zone, pm, total, form }), 2200)
  }
                    ...
                    {PAYMENT_METHODS.map(m => (
                      <button key={m.id} onClick={() => setPm(m.id)} style={{
                        ...
                        <div style={{ width: '40px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <img src={m.iconSrc} alt={m.label} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />
                        </div>
